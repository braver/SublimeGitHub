...
class GitHubOpenCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, *args):
        sublime.status_message('GitHub: running')
        path = self.get_path()
        if not path:
            sublime.status_message('GitHub: No path')
            return False
...
class SideBarGitHubCommand(sublime_plugin.WindowCommand):
